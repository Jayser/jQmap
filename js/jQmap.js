...
(function(){
	...
	var jQmap = {
		...
		el : {
			...
			thumb     	: $('#jQmap-thumb'),
			tools	  	: $('.jQmap-tools'),
			t_createMap : $('#t-createArea'),
			t_save 		: $('#t-save'),
			map 		: '',
			edit_id 	: ''
		},

		// Array for save cordinate maps
		cordinates: [],

		/*
		 * initialize
		 * @Autoresize for showcase
		 * @Events 	   for tools
		 */
		init : function () {
			jQmap.autoresize();
		...
		// @Autoresize image in showcase
		autoresize : function () {

			var showcase = jQmap.el.showcase,
				tools    = jQmap.el.tools,
				thumb    = jQmap.el.thumb,

				thumb_w  = thumb.outerWidth( true ),
				thumb_h  = thumb.outerHeight( true );
				
			showcase.width( thumb_w ).height( thumb_h );
			tools   .width( thumb_w );
		},
		...
		events: function () {
			...
			var enableCreate = jQmap.el.t_createMap,
				save = jQmap.el.t_save,
				root = jQmap.el.root;
			
			// enable create area
			enableCreate.click(function () { jQmap.checking_enable() });

			// save && disable create area
			save.click(function () { jQmap.saveMap() });

			// edit Map
			root.on( "click", '.jQmap-edit' , function(){ jQmap.editMap( $(this) ) });

			// click on outer map area
			$('body').on('click','.imgareaselect-outer',function () { jQmap.checking_enable( 'undo' ); });

		},

		/*
		 * @true 	enable  create map
		 * @false   disable created map
		 */
		checking_enable : function ( status ) {

			var enableCreate   = jQmap.el.t_createMap,
				save   		   = jQmap.el.t_save,
				cord   		   = jQmap.cordinates,
				thumb  		   = jQmap.el.thumb,
				id     		   = jQmap.el.edit_id;

			if ( !enableCreate.hasClass('active') ) {

				enableCreate.addClass('active');
				...
				enableCreate.removeClass('active');
				 ...
				 */	
				status === 'undo' ? jQmap.drawMap( id ) : jQmap.disableMap();		
		...
		enableMap : function () {
			jQmap.el.map = jQmap.el.thumb.imgAreaSelect({ handles: true, instance: true });
		},

		disableMap : function () {
			jQmap.el.thumb.imgAreaSelect({ remove : true });
			jQmap.el.edit_id = '';
		},

		deleteMap : function () {
			// if ( id ) cord.splice( id, 1 );
		},

		// add data to cordinate maps
		saveMap: function () {

			var area = jQmap.el.map.getSelection(),
				id   = jQmap.el.edit_id,
				data = {
					'width'	:area.width,
					'height':area.height,
			...
			if ( area.width !== 0  ) {
				...
				/* find map
				 * if true  replace data
				 * if false add new map
				 */
				id !== '' ? jQmap.cordinates[ id ] = data : jQmap.cordinates.push( data );
		...
		editMap: function ( el ) {
			...
			var el 		= $( el ).closest('.jQmap'),
				area_id = el.attr('data-jQmap'),
				map 	= jQmap.el.map,
				arr_map = jQmap.cordinates[ area_id ];
		...
		drawMap : function ( id ) {
			...
			var cord     = jQmap.cordinates,
				showcase = jQmap.el.showcase,
				data     = cord[ id || cord.length-1 ],
				map      = $('<div></div>'),
				edit     = $('<div class="jQmap-edit"><i class="jQmap-icon jQmap-icon-edit"></i></div>');
			...
			if ( id !== '' || (cord.length-1) !== -1 ){
				...
				map.css({
					...
					top    :( ~~data.y1+10 )+'px'					
				}).addClass('jQmap').attr('data-jQmap', id || cord.length-1 );
